...
import { appShadow, font, FontWeight, AppStyle } from '../../../../../utilis/AppStyle';
import { MainSearchManagerType } from '../../../Manager/MainSearchManager';
interface Props {
  onClose: () => void;
  title: string;
  manager: MainSearchManagerType;
  setSeats: (minPrice: number | null, maxPrice: number | null) => void;
  minPrice? : number | null,
  maxPrice? : number | null
}
const SeatDropDown = (props: Props) => {
    const dispatch = useDispatch()
  const animatedValue = useSharedValue(800);
  const [minPrice, setMinPice] = useState(props?.minPrice ?? 2);
  const [maxPrice, setMaxPrice] = useState(props?.maxPrice ?? 30);
      ...
      <Animated.View
        ...
        <View
          ...
          <View
            ...
            />
            <BorderBtn
              isSelected={minPrice != 2 || maxPrice != 30}
              btnStyle={{
                ...style.btnStyle,
              }}
              onPress={() => {
                if (props.manager.validateBudget(minPrice, maxPrice)) {
                  animateView(false, () => {
                    dispatch(setTabbarVisibility(true))
                    props.setSeats(minPrice, maxPrice);
                  });
                }
              }}
              title="Apply Filter"
...
export default React.memo(SeatDropDown);
